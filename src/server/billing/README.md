...
Subscription billing for ProcureAI. Runs entirely on the Cloudflare Worker; the
...
| `/api/billing/webhook` | POST | Stripe signature | Updates `procureai.subscriptions` |
...
| `STRIPE_PRICE_STARTER` | no | `price_1U5WVCQvnGDiWSOue6LGRe63` |
| `STRIPE_PRICE_PROFESSIONAL` | no | `price_1U5WVRQvnGDiWSOur8epszmi` |
| `STRIPE_PRICE_ENTERPRISE` | no | `price_1U5WVgQvnGDiWSOuBK6Ntvkd` |
| `APP_ORIGIN` | no | Set in the workflow to `https://procure.nexudel.com` |

For local development, copy `.dev.vars.example` to `.dev.vars` (gitignored).

## Stripe webhook setup

Point a webhook endpoint at `https://procure.nexudel.com/api/billing/webhook`
...
- `procureai.subscriptions` grants `authenticated` **SELECT only**, and has a
...
- `public.procureai_subscriptions` is a `security_invoker` view that omits the
